...
public class SpellWordFromSound : IGameMode
{
    ...
    /// </summary>
    Grov∆derSoundController currentWordsoundClip;
    ...
    public void GetSymbols()
    {
        ...
        boardController.SetAnswerText("Tryk [Mellemrum]s tasten for at lytte til Lyden af bogstavet og vÊlg det rigtige.");
    ...
    {
        //bruger correctLetter to find the right sound in tempgrovÊdersound in resource foulder
        ...
        if (clip != null)
        {
            if (currentWordsoundClip == null)
                currentWordsoundClip = GameObject.FindObjectOfType<Grov∆derSoundController>();

            currentWordsoundClip.SetGrovÊderSound(clip); // sends sound to AudioController
        }
        else
        {
