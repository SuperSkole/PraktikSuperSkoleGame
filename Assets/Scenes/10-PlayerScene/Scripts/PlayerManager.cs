...
namespace Scenes._10_PlayerScene.Scripts
{
    ...
    public class PlayerManager : MonoBehaviour
    {
        ...
        {
            PlayerEvents.OnMovePlayerToBlock += MovePlayerToBlock;
        }
        ...
        private void MovePlayerToBlock(Vector3 blockPosition)
        {
            // Implement your movement logic here.
            StartCoroutine(MoveToPosition(blockPosition));
        }
        ...
        private IEnumerator MoveToPosition(Vector3 targetPosition)
        {
            ...
            // After reaching the block, simulate picking it up
            PickUpBlock();
        }

        private void PickUpBlock()
        {
            // Find the block again to ensure it's picked up
            GameObject block = GameObject.Find("WordBlock");
            if (block != null)
            {
                // Attach the block to the player character to simulate carrying it
                block.transform.SetParent(spawnedPlayer.transform);
                block.transform.localPosition = Vector3.zero;  // Adjust position relative to the player if needed

                // After picking up, move to the drop-off point
                MoveToDropOffPoint();
            }
            else
            {
        ...
        {
            StartCoroutine(MoveToPosition(dropOffPoint));
        }

        private void DropOffBlock()
        {
            // Detach the block and place it at the drop-off point
            GameObject block = GameObject.Find("WordBlock");
            if (block != null)
            {
                block.transform.SetParent(null);
                block.transform.position = GetDropOffPoint();  // Set it at the drop-off point

                // Optional: Add any further interaction or feedback here
            }
            else
            {
